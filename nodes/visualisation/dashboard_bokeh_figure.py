...
class SvMegapolisDashboardBokehFigure(SverchCustomTreeNode, bpy.types.Node):
    ...
    def process(self):
        if not self.inputs["Figure Name"].is_linked or \
           not self.inputs["Title"].is_linked or \
           not self.inputs["X Label"].is_linked or \
           not self.inputs["Y Label"].is_linked:
            return
        ...
        figure_plot = (
            f"{figure_name} = figure(title='{title}', "
            f"x_axis_label='{x_label}', y_axis_label='{y_label}')\n\n"
        )
