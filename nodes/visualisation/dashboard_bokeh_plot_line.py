...
class SvMegapolisDashboardBokehPlotLine(SverchCustomTreeNode, bpy.types.Node):
    ...
    def process(self):
        if not (self.inputs["Figure Name"].is_linked and
                self.inputs["Legend Label"].is_linked and
                self.inputs["X Values"].is_linked and
                self.inputs["Y Values"].is_linked):
            return
        ...
        line = (
            f"\n{figure_name}.line({x_values}, {y_values}, "
            f"legend_label='{legend_label}', line_width={line_width})\n"
        )
