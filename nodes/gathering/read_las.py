...
def get_coordinates(las):
    """ Returns coordinates adjusted by scale and offset from LAS file. """
    coordinates = np.stack(
        (
            (las.X * las.header.scales[0]) + las.header.offsets[0],
            (las.Y * las.header.scales[1]) + las.header.offsets[1],
            (las.Z * las.header.scales[2]) + las.header.offsets[2]
        ), axis=1
    )
    return coordinates
...
class SvMegapolisReadLas(SverchCustomTreeNode, bpy.types.Node):
    ...
    def process(self):
        ...
        coordinates = get_coordinates(las)
